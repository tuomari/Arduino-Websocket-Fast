...
bool readByte(Client *socket_client, uint8_t *byte)
{
    return socket_client->read(byte, 1) == 1;
}

bool readShort(Client *socket_client, uint16_t *out)
{
    uint8_t buf[2];
...
bool WebSocketClient::handleStream(uint8_t *data, size_t *data_size, uint8_t *opcode)
{
    ...
    uint16_t length = headers[1] & ~WS_MASK;
    ...
    if (length == WS_SIZE16)
    {
        if (!readShort(socket_client, &length))
        {
            return false;
        }
    }
...
bool WebSocketClient::sendData(const uint8_t *data, const size_t data_size, uint8_t opcode, uint16_t msg_cmd)
{
    int size = data_size;
    ...
    buf[bufPos++] = opcode | WS_FIN;

    // NOTE: no support for > 16-bit sized messages
    if (size > 125)
    {
        buf[bufPos++] = WS_SIZE16 | WS_MASK;
        buf[bufPos++] = size >> 8;
        buf[bufPos++] = size & 0xFF;
    }
    else
    {
        buf[bufPos++] = size | WS_MASK;
    }
    ...
    // Send protocol cmd before data
    if (msg_cmd < 0xffff)
    {
        buf[bufPos++] = ((msg_cmd >> 8) & 0xFF) ^ mask[maskPtr++ % 4];
        buf[bufPos++] = (msg_cmd & 0xFF) ^ mask[maskPtr++ % 4];
    }

    bool middleFlush = false;
    for (int i = 0; i < data_size; ++i)
    {
        ...
        if (bufPos == buf_size)
        {
            ...
            Log.trace(F("%i/%i, "), written, buf_size);
            middleFlush = true;
            if(written != buf_size){
                Log.traceln("");
                Log.warningln("Tried to send %i bytes, but actually sent only %i", buf_size, written);
                return false;
            }
            bufPos = 0;
        }

        buf[bufPos++] = data[i] ^ mask[maskPtr++ % 4];
    }
    

    size_t bytesSent = socket_client->write(buf, bufPos);
    if (middleFlush)
    {
        Log.traceln("");
    }
    Log.traceln("Finishing message with %i bytes. Sent %i, Success %i", bufPos, bytesSent, bufPos == bytesSent);
    return bufPos == bytesSent;
